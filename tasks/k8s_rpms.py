...
(alma/kubernetes/docker-ce): upstream for the k8s test cluster, intranet
mirror for k8s production. dnf only installs missing packages (present packages
are never upgraded), so a converged re-run touches nothing.
"""
